...
int main(void) {
  ...
  int t_fila, fila = 0, ca_atnd = 0, opcao, tanque = 200, retorno;
  /*Tamanho da fila suportada, varíaveis(fila e ca_atnd) de controle da posição
   * dos vetores(Sfila e Sca_atnd), seleção da opção do menu principal,
   * capacidade do tanque de combustíveis, variável para controle de dados de entrada */
  char op_rel; // Seleção da opção no menu relatório
  struct Tcarro *Sfila, *Sca_atnd; /* Vetores do tipo Tcarro que armazenam informações de veículos que estão na fila de espera ou já foram atendidos*/
  ...
  Sca_atnd = (struct Tcarro *)malloc(sizeof(struct Tcarro*));
    ...
    system("clear");
		...
		if(retorno != 0){
		    ...
		    case 3:
		      ...
		      break;
		    case 4:
		      /*Menu de relatórios onde o usúario pode ver o histórico dos atendimentos
		       * realizados*/
		      do {
		        system("clear");
		        ...
		        case 'c':
		          ...
		          printf("%d carros.\n", ca_atnd);
		          imprimir(Sca_atnd, ca_atnd, 1, v_comb);
		          flush_in();
		          espera();
		          break;
		        case 'd':
		          ...
		          break;
		        case 'e':
		          // Chama a função que gera um arquivo .txt com todos os dados do
		          // relatório
		          relatorio(tanque, ca_atnd, v_comb, Sca_atnd, 1);
		          flush_in();
		          espera();
		          break;
		        case 'f':
		    ...
		    case 5:
		      ...
		      relatorio(tanque, ca_atnd, v_comb, Sca_atnd, 0);
		      exit(0);
		      break;
		    default:
